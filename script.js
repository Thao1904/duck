...
const TUNE = {
  ...
  function drawDbg(h){
    ...
    ctx.drawImage(h.image,0,0,debugCanvas.width,debugCanvas.height);
    if (h.multiHandLandmarks){
      for (const lm of h.multiHandLandmarks){
        drawConnectors(ctx,lm,HAND_CONNECTIONS,{color:'#0ff',lineWidth:5});
        drawLandmarks(ctx,lm,{color:'#f0f',lineWidth:2});
      }
    }
    ctx.restore();
  }
